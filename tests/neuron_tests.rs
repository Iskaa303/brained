...
fn test_cpu_neuron_creation_and_tick() {
    ...
    // Initial state check
    assert!((brain_cell.state.soma.v_membrane - -65.0).abs() < f64::EPSILON);
    ...
    assert!(
        (brain_cell.state.soma.v_membrane - -65.0).abs() >= f64::EPSILON,
...
#[cfg(feature = "wgpu")]
#[test]
fn test_wgpu_neuron_creation_and_tick() {
    let wgpu_device = WgpuDevice::default();

    let mut brain_cell = Neuron::<WgpuBackend>::new(&wgpu_device, 1, 2000.0, 5000.0, 1.0, true);
    ...
    // We cannot easily assert internal voltage directly because it's on the GPU,
    // but we know the tick executed without panicking.
}
